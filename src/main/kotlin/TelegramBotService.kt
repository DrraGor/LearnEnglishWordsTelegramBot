...
class TelegramBotService(private val botToken: String) {
    ...
    fun getUpdates(updateId: Long, botToken: String): String {
    ...
    private fun sendStartButton(json: Json, chatId: Long, botToken: String): String {
        val sendMessage = "$HTTP_TELEGRAM$botToken/sendMessage"
        val requestBody = SendMessageRequest(
            chatId = chatId,
            ...
            replyMarkup = ReplyMarkup(
                listOf(
                    listOf(
                        InlineKeyboard(text = "Старт", callbackData = "menu")
                    )
                )
            )
        )
    ...
    fun handleUpdate(update: Update, json: Json, botToken: String, trainers: HashMap<Long, LearnWordsTrainer>) {
        ...
        if (message?.lowercase() == "/start")
            sendStartButton(json, chatId, botToken)
